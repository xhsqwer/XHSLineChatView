...
#define kChartLineColor         [UIColor colorWithRed:109.0/255 green:122.0/255 blue:128.0/255 alpha:1]
#define kChartTextColor         [UIColor colorWithRed:79.0/255 green:79.0/255 blue:79.0/255 alpha:1]


@interface YYAxisView ()
...
- (void)drawRect:(CGRect)rect {
    ...
    for (int i = 0; i < numberOfYAxisElements + 1; i++) {
        ...
        if ([self isPureFloat:self.yMin + avgValue * i]) {
            ...
            if (i == numberOfYAxisElements) {
                [[NSString stringWithFormat:@"%.2f", self.yMin + avgValue * i] drawInRect:CGRectMake(self.frame.size.width - 1-5 - yLabelSize.width, 0, yLabelSize.width, yLabelSize.height) withAttributes:@{NSFontAttributeName :[UIFont systemFontOfSize:8],NSForegroundColorAttributeName:kChartTextColor}];
            }else{
                [[NSString stringWithFormat:@"%.2f", self.yMin + avgValue * i] drawInRect:CGRectMake(self.frame.size.width - 1-5 - yLabelSize.width, self.frame.size.height  - (labelMargin+labelSize.height)* i-labelSize.height*1.5, yLabelSize.width, yLabelSize.height) withAttributes:@{NSFontAttributeName :[UIFont systemFontOfSize:8],NSForegroundColorAttributeName:kChartTextColor}];
            }
        }
        else {
            CGSize yLabelSize = [[NSString stringWithFormat:@"%.0f", self.yMin + avgValue * i] sizeWithAttributes:waterAttr];
            if (i == numberOfYAxisElements) {
                [[NSString stringWithFormat:@"%.0f", self.yMin + avgValue * i] drawInRect:CGRectMake(self.frame.size.width - 1-5 - yLabelSize.width, 0, yLabelSize.width, yLabelSize.height) withAttributes:@{NSFontAttributeName :[UIFont systemFontOfSize:8],NSForegroundColorAttributeName:kChartTextColor}];
            }else{
